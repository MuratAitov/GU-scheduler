...
# Обеспечиваем, что Python знает про директорию выше для импорта credentials
...
def process_prerequisite(graph, prerequisite_schema, parent, full_tree, visited_courses):
    if "course" in prerequisite_schema:
        ...
        # Если включён full_tree - рекурсивно обрабатываем зависимые курсы
        if full_tree:
...
def build_tree_data(course_code, full_tree_mode=False):
    """
    Построить дерево пререквизитов для заданного курса и вернуть структуру в формате JSON.
    """
    graph = nx.DiGraph()
...
# Если нужно протестировать отдельно:
if __name__ == "__main__":
    course_code = "CPSC 346"  # Замените на нужный курс
    full_tree_mode = True
    tree_data = build_tree_data(course_code, full_tree_mode)
